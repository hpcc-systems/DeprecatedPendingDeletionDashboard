...
*/
package org.hpccsystems.dashboard.ajaxbased;
...
public class SidebarAjaxbasedController extends SelectorComposer<Component>{
	...
	public void doAfterCompose(Component comp) throws Exception{
		...
		List<SidebarPage> sideBarPageList = new ArrayList<SidebarPage>(dashboardService.retrieveDashboardMenuPages());
		
		for (Iterator<SidebarPage> iter = sideBarPageList.iterator(); iter.hasNext();) 
		{					
			SidebarPage entry = (SidebarPage) iter.next();						
			Row row = constructSidebarRow(entry.getName(),entry.getLabel(),entry.getIconUri(),entry.getUri());
