...
public class DashboardDaoImpl implements DashboardDao {
    ...
    public int deleteDashboard(final Integer dashboardId, final String userId) throws DataAccessException {
        ...
        rowsdeleted = getJdbcTemplate().update(Queries.DELETE_ACL_PUBLIC, new Object[] { 
                dashboardId
        });
    ...
    public List<Dashboard> retrieveDashboards(String applicationId,
            ...
            dashboardList = getJdbcTemplate().query(Queries.GET_PRIVATE_DASHBOARDS, new DashboardRowMapper(), new Object[] { 
                userId,
                applicationId
            });
 ...
 * @param dashboardList
 */
    private List<Dashboard> getAdminDashboard(List<String> groupCodes, List<Dashboard> dashboardList) {
        ...
        sql.append(")").append(" AND a.role = '").append(Constants.ROLE_ADMIN).append("' ")
        ...
        return getJdbcTemplate().query(sql.toString(), new DashboardRoleRowMapper());
    }
/**
 ...
 * @param dashboardList
 */
    private List<Dashboard> getConsumerDashboard(List<String> groupCodes,List<Dashboard> dashboardList) {
        StringBuilder sql = new StringBuilder();
        sql.append(Queries.GET_ROLE_BASED_DASHBOARDS).append("( '");
        for (String groupCode : groupCodes) {
            sql.append(groupCode).append("'");
 ...
 * @param dashboardList
 */
    private List<Dashboard> getContributorDashboard(List<String> groupCodes,List<Dashboard> dashboardList) {
        StringBuilder sql = new StringBuilder();
        sql.append(Queries.GET_ROLE_BASED_DASHBOARDS).append("( '");
        for (String groupCode : groupCodes) {
            sql.append(groupCode).append("'");
    ...
    public List<Dashboard> getAllDashboard(String applnId) {
        ...
        List<Dashboard> dashboardList = getJdbcTemplate().query(Queries.GET_ALL_DASHBOARD, new DashboardRowMapper(),
                new Object[] { applnId });
        dashboardList = setRole(dashboardList, Constants.ROLE_ADMIN);
        return dashboardList;
    }
    ...
    public Dashboard getDashboard(Integer dashboardId) {
        return getJdbcTemplate().queryForObject(Queries.GET_DASHBOARD, new DashboardRowMapper(),
                new Object[] { dashboardId });
    }

    @Override
    public List<String> getDashboardName(String userId, String applicationId) {
        return getJdbcTemplate().queryForList(Queries.GET_DASHBOARD_NAME, new Object[] { userId, applicationId },
                String.class);
    }
